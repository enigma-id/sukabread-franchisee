...
function NavItem({
  ...
  const isActive =
    location.pathname.split("/").pop() === item.path?.split("/").pop();
          ...
          {item.children!.map((child) => {
            ...
            return (
              <NavLink
                ...
                to={child.path}
                onClick={() => onNavigate()}
                className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-[13px] transition-all duration-150 cursor-pointer group relative overflow-hidden ${isActive ? "bg-primary text-primary-content font-bold shadow-sm" : "text-base-content hover:text-primary hover:bg-base-100/60 font-medium"}`}
              >
                {child.icon && (
                  <span
                    className={`flex-shrink-0 transition-colors ${isActive ? "text-primary-content" : "text-base-content/60 group-hover:text-primary"}`}
